...
//
#ifndef MobilenetV1NoBN_cpp
#define MobilenetV1NoBN_cpp
...
MobilenetV1NoBN::MobilenetV1NoBN(int numClasses, float widthMult, int divisor) {
    ...
    for (int i = 0; i < convSettings.size(); i++) {
        ...
        for (int j = 0; j < times; j++) {
            ...
            convBlocks.emplace_back(DepthwiseSeparableConv2DNoBN({inputChannels, outputChannels}, stride));
    ...
    registerModel({conv1, dropout, fc, conv_ap});
    registerModel(convBlocks);
    setName("MobilenetV1NoBN");
}

std::vector<Express::VARP> MobilenetV1NoBN::onForward(const std::vector<Express::VARP> &inputs) {
