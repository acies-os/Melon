...
Tensor::~Tensor() {
    if (mOwmDynamicMemory) {
        ...
        if (mCacheID == -1 || mDescribe->extra.offset != 0){
            MNN_PRINT("fuck: cacheID=%d\toffset=%lu\n", mCacheID, mDescribe->extra.offset)
        }
        MNN_ASSERT(mCacheID != -1)
        MNN_ASSERT(mDescribe->extra.offset == 0)
