...
bool CPUBackend::allocBuffer(int size, Tensor* dest, StorageType storageType) {
    ...
    if (size > LARGE_MEMORY) {
        MNN_DEBUG_PRINT("Size larger than 500 M :%d\n", size);
    }
    auto& buffer = dest->buffer();
    auto des = TensorUtils::getDescribe(dest);
    std::pair<void*, size_t> points;
    std::string id;
    ...
    MNN_DEBUG_PRINT("\tallocated points.second = %lu for id = %s\n", points.second, id.c_str())
...
bool CPUBackend::onRequireBufferFromOS(const Tensor *nativeTensor) {
    ...
    auto& buffer = dest->buffer();
    auto des = TensorUtils::getDescribe(dest);
    std::pair<void*, int> points = BufferAllocator::allocFromOS(size);
    if (nullptr == points.first) {
        MNN_ERROR("Alloc buffer error for cpu backend\n");
        return false;
    }
...
void CPUBackend::changeBufferType(BufferType bufferType) {
    ...
    if (mBufferType == DYNAMIC_RESIZE) {
        mDynamicResizeID = 0;
    } else if (mBufferType == DYNAMIC_OUTPUT) {
        mDynamicOutputCacheID += 1;
    }
}

void CPUBackend::setHeuristicStrategy(bool flag, std::string modelName, int batchsize, int bgt, bool alignBottom, bool needAlloc) {
    MNN_DEBUG_PRINT("%s: %s, %d\n", __FUNCTION__ ,modelName.c_str(), batchsize);
    mHeuristic = flag;
    if(flag) {
        mDynamicAllocator->setHeuristicStrategy(modelName, batchsize, bgt, alignBottom, needAlloc);
    }
}
...
std::vector<Tensor*> CPUBackend::moveTensor2bottom(std::vector<Tensor *> tensors, size_t bgt_new) {
    return mDynamicAllocator->moveTensor2bottom(tensors, bgt_new);
}

bool CPUBackend::adaptTensorToNewAddress(std::vector<Tensor *> tensors) {
    return mDynamicAllocator->adaptTensorToNewAddress(tensors);
}

size_t CPUBackend::usedSize() {
    return mDynamicAllocator->usedSize();
}
