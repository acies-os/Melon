...
bool MicroSGD::step(Express::VARP loss) {
    mMicroStep++;
    MNN_DEBUG_PRINT("call step in MicroSGD, mMicroStep = %d, mStep = %d\n", mMicroStep, currentStep());
    if (mBatchSize == mMicroBatchSize) {
        MNN_DEBUG_PRINT("don't need to use the micro-batch tech, call father's step directly\n");
    ...
    if (mMicroStep % (mBatchSize / mMicroBatchSize) == 1) {
        ...
        MNN_DEBUG_PRINT("start a new batch, reset gradBuffer\n");
        ...
        {
            AUTOTIME;
            MNN_DEBUG_PRINT("current batch is not the first micro-batch, add grad to buffer\n");
            for(auto& iter: res) {
        ...
        if (mMicroStep % (mBatchSize / mMicroBatchSize) == 0) {
            ...
            MNN_DEBUG_PRINT("end current batch, update params by using gradBuffer\n");
