...
#define MNN_CHECK_CL_SUCCESS(error)                  \
    if (error != CL_SUCCESS) {                       \
        MNN_PRINT("%s:%d\tERROR CODE : %d \n", __FILEPATH__, __LINE__, (int)error); \
    }
...
public:
    ...
    using clCreateSubBufferFunc         = cl_mem (CL_API_CALL *)(cl_mem, cl_mem_flags, cl_buffer_create_type, const void *, cl_int *);
...
#define MNN_CL_DEFINE_FUNC_PTR(func) func##Func func = nullptr
    ...
    MNN_CL_DEFINE_FUNC_PTR(clCreateSubBuffer);
