...
def dump_log_file(model):
    if model == 'Resnet50NoBN':
        batches = (32, 64)
    else:
        batches = (64, 96, 128)

    mnn_log = f'output/mnn/{model}/{model}.{micro_batch}.mnn.out'
    latency = []
    ...
    mnn_throughput = micro_batch * 1000 / min(latency)

    throughput = []
    for batch in batches:
        melon_fn = f'output/ours/{model}/{model}.{batch}.{micro_batch}.ours.out'
        compute_latency = []
        aggregate_latency = []
        with open(melon_fn) as f:
            for line in f:
                if line.startswith('train, 187') and ', cost time:' in line:
                    compute_latency.append(float(line.strip().split(':')[1].split()[0].strip()))
                if line.startswith('step, ') and ', cost time:' in line:
                    aggregate_latency.append(float(line.strip().split(':')[1].split()[0].strip()))
                    # chunked(compute_latency, int(batch / micro_batch))
        # print(len(compute_latency), compute_latency, list(map()))
        # print(len(aggregate_latency), aggregate_latency)
        # compute_latency = list(chunked(compute_latency, int(batch / micro_batch)))
        # print(compute_latency)
        compute_latency = list(map(sum, chunked(compute_latency, int(batch / micro_batch))))
        aggregate_latency = list(map(sum, chunked(aggregate_latency, int(batch / micro_batch) - 1)))
        throughput.append(batch * 1000 / (np.mean(compute_latency) + np.mean(aggregate_latency)))
...
for model in ["MobilenetV2NoBN", "SqueezenetNoBN", "MobilenetV1NoBN", "Resnet50NoBN"]:
    dump_log_file(model)
