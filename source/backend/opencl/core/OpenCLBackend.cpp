...
//        MNN_PRINT("%s:%s:%d: fuck int8 type\n", __FILEPATH__, __FILE_NAME__, __LINE__);
...
//        for (auto t : inputs) {
//            auto tensorShape = OpenCL::tensorShapeFormat(t);
//            mBufferPool->testAlloc(tensorShape[0] * tensorShape[1] * tensorShape[2] * UP_DIV(tensorShape[3], 4) * sizeof(float));
...
//        mBufferPool->testAlloc(32 * sizeof(float));
        return NULL;
    }
...
std::vector<Tensor*> OpenCLBackend::moveTensor2bottom(std::vector<Tensor *> tensors, size_t bgt_new) {
    return {};
}

size_t OpenCLBackend::usedSize() {
    return mBufferPool->usedSize() + mImagePool->usedSize() + mBufferPoolInt8->usedSize();
}
